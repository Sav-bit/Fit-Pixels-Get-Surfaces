...
class INRMetaLearner:
    def __init__(
        ...
        self.model_params = OrderedDict(
            {
                k: v.clone().detach().requires_grad_()
        ...
        self.loss_fn = (
            custom_loss_fn if custom_loss_fn is not None else self.loss_fn_mse
        )
    ...
    def get_inr_parameters(self, key_prefix="inr", copy=True):
        return OrderedDict(
            {
                k: v.clone().detach() if copy else v
    ...
    def adam_update(
        ...
        for (k, p), g in zip(params.items(), grads):
            ...
            # Keep the updated parameter as-is so the computational graph
            # is preserved for higher-order gradients. Calling
            # `requires_grad_()` on a non-leaf tensor raises an error
            # and/or severs the graph needed for second-order MAML.
            updated_params[k] = updated_p
    ...
    def inner_loop(self, coords, data_packet, create_graph=True):
        gt = data_packet["gt"]
        params = OrderedDict(
            (k, v.clone()) for k, v in self.model_params.items()
        )  # initial θ
        ...
        for i in range(self.inner_steps):
            ...
            grads = torch.autograd.grad(
                loss, params.values(), create_graph=not self.first_order
            )

            # Perform Adam update
            lr = self.config.get("inner_lr", 1e-4)
            params, adam_m, adam_v = self.adam_update(
                params, grads, i + 1, adam_m, adam_v, lr=lr
            )
            logging.info(
                f"Inner step: {i + 1}/{self.inner_steps}, Loss: {loss.item():.6f}"
            )
        return params
    ...
    def render_inner_loop(self, coords, gt, inner_loop_steps=1):
        ...
        opt_render = torch.optim.Adam(
            params.values(), lr=self.config.get("inner_lr", 1e-4)
        )
